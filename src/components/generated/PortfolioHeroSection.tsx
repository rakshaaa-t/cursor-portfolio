...
import { X, ArrowUp, Sparkles, Paperclip, Linkedin } from 'lucide-react';
...
export const PortfolioHeroSection: React.FC<PortfolioHeroSectionProps> = ({
  ...
  const scrollToBottom = useCallback(() => {
    setTimeout(() => {
      if (chatContainerRef.current) {
        const messagesDiv = chatContainerRef.current.querySelector('.messages-scroll-area') as HTMLDivElement;
        if (messagesDiv) {
  ...
  const handleDragEnd = useCallback((cardId: string, info: PanInfo) => {
    ...
    if (isInDropZone) {
      ...
      if (card && !messages.some(m => m.type === 'card-with-question' && m.card?.id === cardId)) {
        ...
        // Mark card as used immediately - duplicate appears instantly
        setUsedCardIds(prev => new Set([...prev, cardId]));

        // Trigger scroll after message is added
        setShouldAutoScroll(true);
      }
    }
  }, [cards, messages, scrollToBottom]);
  ...
  return <section aria-label="Portfolio hero" className="relative isolate overflow-hidden bg-[#F2F2F2] text-zinc-900 min-h-screen pb-32">
      ...
      delay: 0.2
        ...
        scale: 0.95
      }} className="flex items-center justify-center w-14 h-14 rounded-full bg-white border border-black/8 shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.12)] transition-all backdrop-blur-sm" aria-label="Say Hi">
        ...
        scale: 0.95
      }} className="flex items-center justify-center w-14 h-14 rounded-full bg-white border border-black/8 shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.12)] transition-all backdrop-blur-sm" aria-label="About Me">
      ...
      delay: 0.2
        ...
        scale: 0.95
      }} className="flex items-center justify-center w-14 h-14 rounded-full bg-white border border-black/8 shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.12)] transition-all text-zinc-900 backdrop-blur-sm" aria-label="Twitter">
          <X className="w-5 h-5" />
        </motion.a>
        ...
        scale: 0.95
      }} className="flex items-center justify-center w-14 h-14 rounded-full bg-white border border-black/8 shadow-[0_8px_24px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.12)] transition-all text-zinc-900 backdrop-blur-sm" aria-label="LinkedIn">
          <Linkedin className="w-5 h-5" />
        </motion.a>
      </motion.div>

      {/* Main Content Container */}
      <div className="relative z-10 mx-auto max-w-[960px] px-4 pb-20 sm:px-6 lg:px-0" style={{
      marginTop: '68px'
    }}>
        <div className="mx-auto flex flex-col items-center">
          ...
          duration: 0.5,
          ease: 'easeOut'
        }} className="font-extrabold tracking-tight leading-tight" style={{
          fontSize: '40px',
          lineHeight: 1.15,
          ...
          {/* Hero Subheading */}
          <h2 className="mt-2 font-extrabold text-zinc-900" style={{
          lineHeight: 1.15,
          textAlign: 'left',
          width: '877px',
          maxWidth: '100%',
          ...
          {/* Hero Description */}
          <p className="mt-6 text-[14px] leading-6 text-zinc-700" style={{
          textAlign: 'left',
          width: '877px',
          maxWidth: '100%'
        }}>
          ...
          {/* Chat Container */}
          <div ref={chatContainerRef} className="mt-8 w-full rounded-[14px] border border-black/8 bg-white p-2 shadow-[0_8px_32px_rgba(0,0,0,0.06)] transition-all" style={{
          width: '877px',
          maxWidth: '100%',
          ...
          borderColor: draggedOverChat ? '#1D3BF1' : 'rgba(0,0,0,0.08)'
            ...
            {/* Messages Area */}
            <div className="messages-scroll-area rounded-[12px] border border-black/8 bg-white px-4 py-4 max-h-[400px] overflow-y-auto space-y-3">
              <AnimatePresence mode="popLayout">
                ...
                ease: 'easeOut'
                    ...
                    {message.sender === 'system' && <div className="flex items-start gap-3 max-w-xs">
                        <img src="https://storage.googleapis.com/storage.magicpath.ai/user/323295203727400960/assets/a162f3c9-9017-4e52-a2b7-d48614b32b0f.jpg" alt="Raksha avatar" className="size-8 rounded-full object-cover flex-shrink-0" style={{
            ...
            backgroundColor: draggedCardId ? '#f8f9ff' : 'white'
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-1 items-center gap-3">
                  <Sparkles className="h-5 w-5 text-zinc-400" strokeWidth={2} />
                  <input type="text" value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSendMessage()} placeholder="Ask me about myself, my case studies, or my process." className="flex-1 bg-transparent text-[15px] text-zinc-900 placeholder:text-zinc-400 focus:outline-none" aria-label="Chat input" />
                </div>
              ...
              duration: 0.2
            }} className="mt-3 flex items-center justify-center gap-2 rounded-[12px] border-2 border-dashed border-[#1D3BF1]/30 bg-[#1D3BF1]/5 py-3 text-[13px] font-medium text-[#1D3BF1]">
          ...
          duration: 0.6,
          delay: 0.4
        }} className="relative mt-16 mx-auto" style={{
          width: '877px',
          maxWidth: '100%',
          minHeight: '360px'
        }}>
            ...
            <div className="relative w-full flex items-start justify-center flex-wrap gap-6 px-4">
          ...
          maxWidth: '100%'
            ...
            borderBottom: '1px solid rgba(0, 0, 0, 0.06)'
              ...
              <ul className="flex items-center justify-center gap-8 list-none p-0 m-0">
                {TAB_ITEMS.map(tab => <li key={tab.id} className="m-0 p-0">
                  ...
                  e.preventDefault();
                  setActiveTab(tab.id);
                }} className="relative inline-block px-0 py-4 text-base font-normal transition-colors duration-200 focus:outline-none" style={{
            ...
            duration: 0.4,
            ease: 'easeInOut'
          }} className="mt-16 space-y-6 mx-auto" style={{
            width: '877px',
            maxWidth: '100%'
          }}>
              ...
              duration: 0.4,
              ease: 'easeOut'
            }} className={`group rounded-[18px] overflow-hidden border transition-all hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] cursor-pointer ${caseStudy.backgroundColor} ${caseStudy.borderColor} shadow-[0_8px_30px_rgba(0,0,0,0.06)]`}>
                    {/* Image Container */}
                    <div className="relative w-full overflow-hidden bg-zinc-100" style={{
                aspectRatio: '180 / 140'
              }}>
